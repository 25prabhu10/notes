...
1. HTML - The content - _Noun_
2. CSS - Presentation of the content - _Adjective_
3. JavaScript - Interactions of the content - _Verb_
...
**Statements** _do things._ A program is a sequence of statements. Here is an example of a statement, which declares (creates) a variable foo:
...
::: tip NOTE
Truthy values: all none Falsy values
...
- `==` is an abstract comparison operator. It will **typecast before comparison**.
...
1. **Boolean**: Logical data type that can only be _true_ or _false_.
2. **Null**: Empty value assigned by the user. Also means 'non-existent'. No value is present.
3. **Undefined** - Default value of a variable until a value is assigned to it.
4. **Number**: Floating point numbers for both decimals and integers.
5. **BigInt**:
...
Objects are mutable. If a value is not a Primitive then it is an Object.
